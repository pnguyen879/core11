...
.EXAMPLE
    .\core11basic.ps1 E D
    .\core11basic.ps1 -ISO E -SCRATCH D
    .\core11basic.ps1 -SCRATCH D -ISO E
    .\core11basic.ps1
...
{
    Write-Output "Restarting core11 image creator as admin in a new window, you can close this one."
...
if (-not (Test-Path -Path "$PSScriptRoot/autounattend.xml")) {
    Invoke-RestMethod "https://raw.githubusercontent.com/ntdevlabs/core11builder/refs/heads/main/autounattend.xml" -OutFile "$PSScriptRoot/autounattend.xml"
}

# Start the transcript and prepare the window
Start-Transcript -Path "$PSScriptRoot\core11_$(get-date -f yyyyMMdd_HHmms).log"

$Host.UI.RawUI.WindowTitle = "core11 image creator"
Clear-Host
Write-Output "Welcome to the core11 image creator! Release: 09-07-25"

$hostArchitecture = $Env:PROCESSOR_ARCHITECTURE
New-Item -ItemType Directory -Force -Path "$ScratchDisk\core11\sources" | Out-Null
do {
    if (-not $ISO) {
...
if ((Test-Path "$DriveLetter\sources\boot.wim") -eq $false -or (Test-Path "$DriveLetter\sources\install.wim") -eq $false) {
    if ((Test-Path "$DriveLetter\sources\install.esd") -eq $true) {
        ...
        Export-WindowsImage -SourceImagePath $DriveLetter\sources\install.esd -SourceIndex $index -DestinationImagePath $ScratchDisk\core11\sources\install.wim -Compressiontype Maximum -CheckIntegrity
    } else {
...
Copy-Item -Path "$DriveLetter\*" -Destination "$ScratchDisk\core11" -Recurse -Force | Out-Null
Set-ItemProperty -Path "$ScratchDisk\core11\sources\install.esd" -Name IsReadOnly -Value $false > $null 2>&1
Remove-Item "$ScratchDisk\core11\sources\install.esd" > $null 2>&1
Write-Output "Copy complete!"
Start-Sleep -Seconds 2
Clear-Host
Write-Output "Getting image information:"
$ImagesIndex = (Get-WindowsImage -ImagePath $ScratchDisk\core11\sources\install.wim).ImageIndex
while ($ImagesIndex -notcontains $index) {
    Get-WindowsImage -ImagePath $ScratchDisk\core11\sources\install.wim
    $index = Read-Host "Please enter the image index"
}
Write-Output "Mounting Windows image. This may take a while."
$wimFilePath = "$ScratchDisk\core11\sources\install.wim"
...
Mount-WindowsImage -ImagePath $ScratchDisk\core11\sources\install.wim -Index $index -Path $ScratchDisk\scratchdir
...
$imageInfo = & 'dism' '/English' '/Get-WimInfo' "/wimFile:$($ScratchDisk)\core11\sources\install.wim" "/index:$index"
...
Dism.exe /Export-Image /SourceImageFile:"$ScratchDisk\core11\sources\install.wim" /SourceIndex:$index /DestinationImageFile:"$ScratchDisk\core11\sources\install2.wim" /Compress:recovery
Remove-Item -Path "$ScratchDisk\core11\sources\install.wim" -Force | Out-Null
Rename-Item -Path "$ScratchDisk\core11\sources\install2.wim" -NewName "install.wim" | Out-Null
Write-Output "Windows image completed. Continuing with boot.wim."
Start-Sleep -Seconds 2
Clear-Host
Write-Output "Mounting boot image:"
$wimFilePath = "$ScratchDisk\core11\sources\boot.wim"
& takeown "/F" $wimFilePath | Out-Null
& icacls $wimFilePath "/grant" "$($adminGroup.Value):(F)"
Set-ItemProperty -Path $wimFilePath -Name IsReadOnly -Value $false
Mount-WindowsImage -ImagePath $ScratchDisk\core11\sources\boot.wim -Index 2 -Path $ScratchDisk\scratchdir
Write-Output "Loading registry..."
...
Dismount-WindowsImage -Path $ScratchDisk\scratchdir -Save
Clear-Host
Write-Output "The core11 image is now completed. Proceeding with the making of the ISO..."
Write-Output "Copying unattended file for bypassing MS account on OOBE..."
Copy-Item -Path "$PSScriptRoot\autounattend.xml" -Destination "$ScratchDisk\core11\autounattend.xml" -Force | Out-Null
...
& "$OSCDIMG" '-m' '-o' '-u2' '-udfver102' "-bootdata:2#p0,e,b$ScratchDisk\core11\boot\etfsboot.com#pEF,e,b$ScratchDisk\core11\efi\microsoft\boot\efisys.bin" "$ScratchDisk\core11" "$PSScriptRoot\core11.iso"

# Finishing up
Write-Output "Creation completed! Press any key to exit the script..."
Read-Host "Press Enter to continue"
Write-Output "Performing Cleanup..."
Remove-Item -Path "$ScratchDisk\core11" -Recurse -Force | Out-Null
...
if (Test-Path -Path "$ScratchDisk\core11") {
    Write-Output "core11 folder still exists. Attempting to remove it again..."
    Remove-Item -Path "$ScratchDisk\core11" -Recurse -Force -ErrorAction SilentlyContinue
    if (Test-Path -Path "$ScratchDisk\core11") {
        Write-Output "Failed to remove core11 folder."
    } else {
        Write-Output "core11 folder removed successfully."
    }
} else {
    Write-Output "core11 folder does not exist. No action needed."
}
