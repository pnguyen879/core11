...
{
    Write-Host "Restarting core11 image creator as admin in a new window, you can close this one."
...
Start-Transcript -Path "$PSScriptRoot\core11.log" 
# Ask the user for input
Write-Host "Welcome to core11 minimal builder! BETA 09-05-25"
Write-Host "This script generates a significantly reduced Windows 11 image. However, it's not suitable for regular use due to its lack of serviceability - you can't add languages, updates, or features post-creation. core11 Core is not a full Windows 11 substitute but a rapid testing or development tool, potentially useful for VM environments."
...
New-Item -ItemType Directory -Force -Path "$mainOSDrive\core11\sources" >null
...
if ((Test-Path "$DriveLetter\sources\boot.wim") -eq $false -or (Test-Path "$DriveLetter\sources\install.wim") -eq $false) {
    if ((Test-Path "$DriveLetter\sources\install.esd") -eq $true) {
        ...
        & 'DISM' /Export-Image /SourceImageFile:"$DriveLetter\sources\install.esd" /SourceIndex:$index /DestinationImageFile:"$mainOSDrive\core11\sources\install.wim" /Compress:max /CheckIntegrity
    } else {
...
Copy-Item -Path "$DriveLetter\*" -Destination "$mainOSDrive\core11" -Recurse -Force > null
Set-ItemProperty -Path "$mainOSDrive\core11\sources\install.esd" -Name IsReadOnly -Value $false > $null 2>&1
Remove-Item "$mainOSDrive\core11\sources\install.esd" > $null 2>&1
Write-Host "Copy complete!"
Start-Sleep -Seconds 2
Clear-Host
Write-Host "Getting image information:"
&  'dism' '/English' "/Get-WimInfo" "/wimfile:$mainOSDrive\core11\sources\install.wim"
$index = Read-Host "Please enter the image index"
Write-Host "Mounting Windows image. This may take a while."
$wimFilePath = "$($env:SystemDrive)\core11\sources\install.wim" 
...
& dism /English "/mount-image" "/imagefile:$($env:SystemDrive)\core11\sources\install.wim" "/index:$index" "/mountdir:$($env:SystemDrive)\scratchdir"
...
$imageInfo = & 'dism' '/English' '/Get-WimInfo' "/wimFile:$($env:SystemDrive)\core11\sources\install.wim" "/index:$index"
...
if ($input -eq 'y') {
    ...
    & 'dism'  "/image:$scratchDir" '/enable-feature' '/featurename:NetFX3' '/All' "/source:$($env:SystemDrive)\core11\sources\sxs" 
...
& 'dism' '/English' '/Export-Image' "/SourceImageFile:$mainOSDrive\core11\sources\install.wim" "/SourceIndex:$index" "/DestinationImageFile:$mainOSDrive\core11\sources\install2.wim" '/compress:max'
Remove-Item -Path "$mainOSDrive\core11\sources\install.wim" -Force >null
Rename-Item -Path "$mainOSDrive\core11\sources\install2.wim" -NewName "install.wim" >null
Write-Host "Windows image completed. Continuing with boot.wim."
Start-Sleep -Seconds 2
Clear-Host
Write-Host "Mounting boot image:"
$wimFilePath = "$($env:SystemDrive)\core11\sources\boot.wim" 
& takeown "/F" $wimFilePath >null
& icacls $wimFilePath "/grant" "$($adminGroup.Value):(F)"
Set-ItemProperty -Path $wimFilePath -Name IsReadOnly -Value $false
& 'dism' '/English' '/mount-image' "/imagefile:$mainOSDrive\core11\sources\boot.wim" '/index:2' "/mountdir:$mainOSDrive\scratchdir"
Write-Host "Loading registry..."
...
& dism /Export-Image /SourceImageFile:"$mainOSDrive\core11\sources\install.wim" /SourceIndex:1 /DestinationImageFile:"$mainOSDrive\core11\sources\install.esd" /Compress:recovery
Remove-Item "$mainOSDrive\core11\sources\install.wim" > $null 2>&1
Write-Host "The core11 image is now completed. Proceeding with the making of the ISO..."
...
Remove-Item -Path "$mainOSDrive\core11" -Recurse -Force >null
